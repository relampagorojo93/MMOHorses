...
public class SpeedLimitFlag {
	StateFlag flag;

	public SpeedLimitFlag() {
		FlagRegistry fr;
		try {
		...
		try {
			StateFlag f = new StateFlag("restrict-horse-speed", false);
			fr.register(f);
			flag = f;
		} catch (FlagConflictException e) {
			MMOHorsesAPI.getDebugController().addDebugData(new DebugAlertData("Seems like another flag is conflicting with MMOHorses' restrict-horse-speed flag."));
		}
	}
	...
	public boolean flagState(Player pl) {
		if (flag == null || pl.hasPermission("horse.speedbypass"))
			return true;
