...
public class MuleCustom extends EntityHorseMule implements NMSMMOHorse {
	...
	public MuleCustom(EntityTypes<? extends EntityHorseMule> entitytypes, World w) { // Required
		super(entitytypes, w);
		data.initialize();
	}

	public MuleCustom(Location l) {
		this(EntityTypes.MULE, ((CraftWorld) l.getWorld()).getHandle());
	...
	@Override
	public MuleCustom createEntity(org.bukkit.World world) {
		return createEntity(((CraftWorld) world).getHandle());
	}

	public MuleCustom createEntity(World world) {
		return new MuleCustom(EntityTypes.MULE, world);
	}
