...
public class SkeletonHorseCustom extends EntityHorseSkeleton implements NMSMMOHorse {
	...
	public SkeletonHorseCustom(EntityTypes<? extends EntityHorseSkeleton> entitytypes, World w) { // Required
		super(entitytypes, w);
		data.initialize();
	}

	public SkeletonHorseCustom(EntityTypes<? extends EntityHorseSkeleton> entitytypes, Location l) { // Required
		this(entitytypes, ((CraftWorld) l.getWorld()).getHandle());
		this.setLocation(l.getX(), l.getY(), l.getZ(), l.getYaw(), l.getPitch());
		((CraftWorld) l.getWorld()).getHandle().addEntity(this);
	}

	public SkeletonHorseCustom(Location l) {
		this(EntityTypes.SKELETON_HORSE, l);
	}
	...
	public Type getType() {
		return Type.SKELETONHORSE;
	}

	@Override
	public SkeletonHorseCustom createEntity(org.bukkit.World world) {
		return createEntity(((CraftWorld) world).getHandle());
	}

	public SkeletonHorseCustom createEntity(World world) {
		return new SkeletonHorseCustom(EntityTypes.SKELETON_HORSE, world);
	}
