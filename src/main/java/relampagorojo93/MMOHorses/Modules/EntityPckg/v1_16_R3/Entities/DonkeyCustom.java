...
public class DonkeyCustom extends EntityHorseDonkey implements NMSMMOHorse {
	...
	public DonkeyCustom(EntityTypes<? extends EntityHorseDonkey> entitytypes, World w) { // Required
		super(entitytypes, w);
		data.initialize();
	}

	public DonkeyCustom(Location l) {
		this(EntityTypes.DONKEY, ((CraftWorld) l.getWorld()).getHandle());
	...
	@Override
	public DonkeyCustom createEntity(org.bukkit.World world) {
		return createEntity(((CraftWorld) world).getHandle());
	}

	public DonkeyCustom createEntity(World world) {
		return new DonkeyCustom(EntityTypes.DONKEY, world);
	}
