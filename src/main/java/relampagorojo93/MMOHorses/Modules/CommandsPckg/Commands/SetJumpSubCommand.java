...
public class SetJumpSubCommand extends SubCommand {
	public SetJumpSubCommand(Command command) {
		super(command, "setjump", SettingString.HORSE_SETJUMP_NAME.toString(), SettingString.HORSE_SETJUMP_PERMISSION.toString(),
				SettingString.HORSE_SETJUMP_DESCRIPTION.toString(), SettingString.HORSE_SETJUMP_PARAMETERS.toString(),
				SettingList.HORSE_SETJUMP_ALIASES.toList());
	}
				...
				if (ec != null) {
					if (args.length >= 2) {
						try {
							...
							ec.getMMOHorseData().setJump(d, true);
