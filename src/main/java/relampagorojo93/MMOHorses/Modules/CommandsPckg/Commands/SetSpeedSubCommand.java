...
public class SetSpeedSubCommand extends SubCommand {
	public SetSpeedSubCommand(Command command) {
		super(command, "setspeed", SettingString.HORSE_SETSPEED_NAME.toString(), SettingString.HORSE_SETSPEED_PERMISSION.toString(),
				SettingString.HORSE_SETSPEED_DESCRIPTION.toString(), SettingString.HORSE_SETSPEED_PARAMETERS.toString(),
				SettingList.HORSE_SETSPEED_ALIASES.toList());
	}
				...
				if (ec != null) {
					if (args.length >= 2) {
						try {
							...
							ec.getMMOHorseData().setSpeed(d, true);
